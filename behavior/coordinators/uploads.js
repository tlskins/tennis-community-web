...
const s3 = new AWS.S3({
  accessKeyId: (process.env.VERCEL_GIT_COMMIT_REF === "production" || process.env.VERCEL_GIT_COMMIT_REF === undefined)
    ? process.env.NEXT_PUBLIC_PROD_AWS_ACCESS_KEY
    : process.env.NEXT_PUBLIC_AWS_ACCESS_KEY,
  secretAccessKey: (process.env.VERCEL_GIT_COMMIT_REF === "production" || process.env.VERCEL_GIT_COMMIT_REF === undefined)
...
export const UploadVideo = (dispatch, callback = () => {}) => async ({
  ...
  try {
    ...
    const params = {
      Bucket: (process.env.VERCEL_GIT_COMMIT_REF === "production" || process.env.VERCEL_GIT_COMMIT_REF === undefined)
    ...
    s3.upload(params, async (err, data) => {
      ...
      dispatch(newNotification({ message: `Processing new album: "${albumName}" ETA 5-10 minutes (${Moment().add(10, "minutes").format("h:mm a")})` }))
